...
(define (domain thoughtful-untyped)
	(:predicates 
		(card ?c)
		(colnum ?n)
		(num ?n)
...
;; move card from one column to another - create an empty column
	...
	(:action move-col-to-col-b
		...
		:precondition (and (card ?card) (card ?newcard)
				(colnum ?cols) (colnum ?ncols)
				(faceup ?card)
				(bottomcol ?card)
				(clear ?newcard)
				(canstack ?card ?newcard)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action col-to-home
		...
		:precondition (and (card ?card) (card ?oldcard)
			(suit-type ?suit) (num ?vcard) (card ?homecard)
			(num ?vhomecard)
			(clear ?card)
			(on ?card ?oldcard)
			(home ?homecard)
...
;; Move card from column to home - create an empty column
	...
	(:action col-to-home-b
		:parameters (?card ?homecard ?suit ?vcard  
			?vhomecard ?cols ?ncols)
		:precondition (and 
			(card ?card) (card ?homecard) (suit-type ?suit) (num ?vcard)
			(num ?vhomecard) (colnum ?cols) (colnum ?ncols)
			(clear ?card)
			(bottomcol ?card)
			(home ?homecard)
			(suit ?card ?suit)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;
	...
	(:action tal-to-col
		:parameters (?card ?oldcard ?newcard ?cardabove)
		:precondition (and (card ?card) (card ?oldcard)
...
;; move card from talon to column - card is bottom card in talon
	...
	(:action tal-to-col-b
		:parameters (?card ?newcard ?cardabove)
		:precondition (and (card ?card) (card ?newcard)
			(card ?cardabove)
			(clear ?newcard)
			(ontalon ?cardabove ?card)
...
;; move card from talon to column - card is top card in talon
	...
	(:action tal-to-col-c
		:parameters (?card ?newcard ?oldcard)
		:precondition (and
			(card ?card) (card ?newcard) (card ?oldcard)
			(clear ?newcard)
			(ontalon ?card ?oldcard)
			(canstack ?card ?newcard)
			(talonplayable ?card)
...
;; move king from talon to column
	...
	(:action tal-to-col-e
		:parameters (?card ?oldcard ?cardabove ?cols ?ncols)
		:precondition (and 
			(card ?card) (card ?oldcard) (card ?cardabove)
			(colnum ?cols) (colnum ?ncols)
			(king ?card)
			(ontalon ?card ?oldcard)
			(ontalon ?cardabove ?card)
...
;; move king from talon to column - king is bottom card in talon
	...
	(:action tal-to-col-f
		:parameters (?card ?cardabove ?cols ?ncols)
		:precondition (and
			(card ?card) (card ?cardabove)
			(colnum ?cols) (colnum ?ncols)
			(king ?card)
			(ontalon ?cardabove ?card)
			(talonplayable ?card)
...
;; move card from talon to home
	...
	(:action tal-to-home
		:parameters (?card ?cardabove ?cardbelow ?homecard
			?cardsuit ?vcard ?vhomecard)
		:precondition (and
			(card ?card) (card ?cardabove) (card ?cardbelow)
 			(card ?homecard)
			(suit-type ?cardsuit)
			(num ?vcard) (num ?vhomecard)
			(talonplayable ?card)
			(ontalon ?cardabove ?card)
...
;; move card from talon to home - card is bottom card in talon
	...
	(:action tal-to-home-b
		:parameters (?card ?cardabove ?homecard ?cardsuit 
			?vcard ?vhomecard)
		:precondition (and
			(card ?card) (card ?cardabove) (card ?homecard)
			(suit-type ?cardsuit) (num ?vcard) (num ?vhomecard)
			(talonplayable ?card)
			(ontalon ?cardabove ?card)
			(bottomtalon ?card)
			(home ?homecard)
...
;; move card from talon to home - card is top card in talon
	...
	(:action tal-to-home-c
		:parameters (?card ?cardbelow ?homecard ?cardsuit
			?vcard ?vhomecard)
		:precondition (and
			(card ?card) (card ?cardbelow) (card ?homecard)
			(suit-type ?cardsuit) (num ?vcard) (num ?vhomecard)
			(ontalon ?card ?cardbelow)
			(talonplayable ?card)
			(toptalon ?card)
...
;; move card from talon to home - card is the only card in talon
	...
	(:action tal-to-home-d
		:parameters (?card ?homecard ?cardsuit 
			?vcard ?vhomecard)
		:precondition (and
			(card ?card) (card ?homecard)
			(suit-type ?cardsuit) (num ?vcard) (num ?vhomecard)
			(bottomtalon ?card)
			(toptalon ?card)
			(talonplayable ?card)
...
;; move card from home to column
	...
	(:action home-to-col
		:parameters (?card ?cardbelow ?newcard ?cardsuit
			?vcard ?vcardbelow)
		:precondition (and
			(card ?card) (card ?cardbelow) (card ?newcard)
			(suit-type ?cardsuit) (num ?vcard) (num ?vcardbelow)
			(home ?card)
			(suit ?card ?cardsuit)
...
;; move king from home to column
	...
	(:action home-to-col-a
		:parameters (?card ?cardbelow ?cardsuit
			?vcard ?vcardbelow ?cols ?ncols)
		:precondition (and
			(card ?card) (card ?cardbelow) 
			(suit-type ?cardsuit) (num ?vcard) (num ?vcardbelow)
			(colnum ?cols) (colnum ?ncols)
			(home ?card)
			(king ?card)
