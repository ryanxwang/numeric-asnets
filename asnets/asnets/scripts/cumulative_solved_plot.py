...
parser.add_argument('--num-problems',
                    type=int,
                    default=None,
                    help='number of problems, will use unique problem names '
                    'if not given')
parser.add_argument('--no-train-hours',
                    action='store_false',
                    dest='use_train_hours',
                    help='disable use of training hours in plot')
    ...
    for name, size, data_dict in zip(data['eval_names'], data['eval_sizes'],
        ...
        for seq_num, (goal_reached, cost, time) in enumerate(
            ...
            if time is None:
                ...
                if args.use_train_hours:
                    time_or_timeout = time + train_time
                else:
                    time_or_timeout = time
            record = {
                "problem": name,
                "problem_size": size,
                "method": label,
...
def main():
    ...
    for method in data_frame['method'].unique():
        ...
        if train_time > 0 and args.use_train_hours:
        ...
        ctime = train_time if args.use_train_hours else 0
        ccov = 0
    ...
    ctime_frame['IsBaseline'] = ctime_frame['method'].apply(
        lambda x: not 'ASNets' in x)
    ax = sns.lineplot(data=ctime_frame,
                 x="hours",
                 y="coverage",
                 hue="Method",
                 drawstyle='steps-pre',
                 style='IsBaseline',   # dotted lines for baselines
                 legend=args.add_legend and 'brief')
    sns.move_legend(ax, loc='upper right', bbox_to_anchor=(1.0, 1.0), ncol=1)
    ...
    if args.train_size is not None:
        plt.axhline(args.train_size, ls='--', color='cyan')
