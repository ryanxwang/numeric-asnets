...
def print_expt_results(full_results):
    ...
    print(' <tr><th>Problem</th><th>Success rate</th>'
          '<th>Mean successful trial cost</th><th>Evaluation time</th>'
...
def parse_results_dir(results_dir):
    ...
    if 'final' not in results_dir_bn:
        expt_mod_name, arch_mod_name = results_dir_bn.split('-')[:2]
        expt_name = expt_mod_name.split('.')[-1]
        arch_name = arch_mod_name.split('.')[-1]
    else:
        if 'block_grouping' in results_dir_bn:
            arch_name = 'block_grouping'
        elif 'fo_counters' in results_dir_bn:
            arch_name = 'fo_counters'
        elif 'fn_counters' in results_dir_bn:
            arch_name = 'fn_counters'
        else:
            arch_name = results_dir_bn.split('.')[-1].split('_')[0]
        
        expt_name = results_dir_bn.split('.')[-1].replace(arch_name + '_', '').replace('_final', '')
    ...
    for subdir in subdirs:
        ...
        try:
            ...
            print("Couldn't find results.json in '{}'; {}; skipping".format(
                subdir,
                'Evaluation timed out' if term_stat['timed_out'] else 'Did not time out'))
            if not is_train:
                continue
        ...
        if results_dict.get('val_checked', False):
            # replace the None costs with 0
            for i in range(results_dict['trials']):
                if results_dict['all_costs'][i] is None:
                    results_dict['all_costs'][i] = 0
