...
from asnets.utils.pddl_utils import relax_numeric_pddls
...
def set_up_ssipp(ssipp_module: ModuleType,
                 pddl_paths: List[str],
                 problem_name: str,
                 use_numeric_relaxation: bool = False) -> Any:
    """Parse some PDDL files and return a problem from SSiPP.

    Args:
        ssipp_module (ModuleType): The SSiPP Python module.
        pddl_paths (List[str]): Paths to PDDL files.
        problem_name (str): Name of the problem to initialise.

    Returns:
        Any: Reference to the problem object from SSiPP, of type *problem_t.
    """
    if not use_numeric_relaxation:
        problem_paths = pddl_paths
    else:
        # this means we relax the PDDLs once for each problem... which is bad
        # but acceptable for now
        problem_paths = relax_numeric_pddls(pddl_paths)
...
class Evaluator:
    ...
    def __init__(self, planner_exts: 'PlannerExtensions',
                 heuristic_name: str):
        """Create an evaluator for a given heuristic via SSiPP.
    ...
    def eval_state(self, ssipp_state: Any) -> float:
        """Evaluate a state.
    ...
    def succ_probs_vals(self, ssipp_state: Any, action_name: str) \
            -> List[Tuple[float, float]]:
        """Get the successor probabilities and values for a given action.
