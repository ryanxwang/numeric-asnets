...
from asnets.interfaces.metricff_interface import MetricFFQValueCache
...
class Teacher(metaclass=ABCMeta):
    ...
    def dead_end_value(self):
        ...
        Returns:
            float: Cutoff value for maximum length of a path to reach goal.
        """
        pass
    ...
    def _blacklist_decorator(inner_func):
        ...
        def wrapper_func(self, cstate, *args, **kwargs):
            ...
            try:
                ...
                raise TeacherException('SSiPP timeout: %s' % ex)
            finally:
...
class FDTeacher(Teacher):
    ...
    def __init__(self, planner_exts: 'PlannerExtensions', heuristic: str, *,
        ...
        """
        self._plan_exts = planner_exts
        self._qv_cache = FDQValueCache(
            planner_exts, planner=heuristic, timeout_s=timeout_s)
    ...
    def q_values(self, cstate: CanonicalState, act_strs: List[str]) \
        ...
        Raises:
            TeacherException: If the FD timeout is reached.

        Returns:
            List[float]: List of Q-values, in the same order as the actions.
        """
        try:
            q_values = self._qv_cache.compute_q_values(cstate, act_strs)
        except FDTimeout as ex:
            raise TeacherException("FDTimeout: %s" % ex)

        return list(map(
            lambda qv: qv if qv is not None else self.dead_end_value, q_values))
    ...
    def extract_policy_envelope(self, cstate: CanonicalState) \
        ...
        TeacherException raised instead of FDTImeout.
...
class ENHSPTeacher(Teacher):
    """Wraps unified-planning as a teacher planner."""
    ...
    def __init__(self,
                 ...
                 *,
                 enhsp_config = None):
        self._plan_exts = planner_exts
        self._teacher_cache = ENHSPCache(
            self._plan_exts,
    ...
    def q_values(self, cstate: CanonicalState, act_strs: List[str]) \
        ...
        Returns:
            List[float]: A list of Q-values of same length as `act_strs`, one
                corresponding to each action in the given state.
        """
        q_values = self._teacher_cache.compute_q_values(cstate, act_strs)
    ...
    def single_action_label(self, cstate: CanonicalState) -> str:
        ...
        """
        action_sans_parens = \
            self._teacher_cache.compute_state_value(cstate,
                                                    need_best_action=True)[1]
        return f'({action_sans_parens})'
    ...
    def extract_policy_envelope(self, cstate: CanonicalState) \
        ...
        Raises:
            TeacherException: If ENHSP fails to find a policy for the given
            state.
        ...
        Returns:
            Iterable[CanonicalState]: Iterable of visited canonical states.
        """
        return self._teacher_cache.compute_policy_envelope(cstate)
...
class MetricFFTeacher(Teacher):
    """Wraps MetricFF as a teacher planner."""

    def __init__(self,
                 planner_exts: 'PlannerExtensions',
                 timeout_s: int):
        self._plan_exts = planner_exts
        self._teacher_cache = MetricFFQValueCache(
            self._plan_exts,
            timeout_s=timeout_s)
