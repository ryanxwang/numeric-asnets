...
def run_once(f: Callable) -> Callable:
    """Decorator that ensures a function is only run once. Subsequent calls
    will do nothing and return None.

    Args:
        f (Callable): Functiont to decorate.
...
def get_files_with_extension(folder_path: str, extension: str) -> List[str]:
    """Get all files in folder_path with the given extension.

    Args:
        folder_path (str): path to folder to search.
        extension (str): extension to search for (e.g. '.txt').

    Returns:
        List[str]: list of paths to files with the given
...
def weak_ref_to(obj: Any) -> Any:
    ...
    Returns:
        Any: weak reference to object, or object if it was
...
def strip_parens(thing):
    ...
    assert (
        len(thing) > 2 and thing[0] == "(" and thing[-1] == ")"
...
class TimerContext:
    ...
    def time_block(self, timer_name):
        ...
        try:
            ...
            new_mean = old_mean * (old_count / new_count_f) + elapsed / new_count_f
...
class RandomPopContainer:
    """Container for achieving O(1) insertion and O(1) random removal while
    ...
    def add(self, element):
        ...
        self.index_map.setdefault(element, set()).add(index)

    def pop_random(self):
        if len(self.data) == 0:
            raise ValueError("pop from empty container")
        ...
        self.index_map[element].remove(index)
        self.index_map[last_element].add(index)
        self.index_map[last_element].remove(len(self.data) - 1)

        # Remove last element
        self.data.pop()
        if not self.index_map[last_element]:
            del self.index_map[last_element]
